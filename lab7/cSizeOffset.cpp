...
void cSizeOffset::Visit(cVarDeclNode *node)
{
    ...
    cBaseTypeNode *baseType = dynamic_cast<cBaseTypeNode*>(type);
    if (baseType != nullptr)
    {
        node->SetSize(baseType->GetSize());
        return;
    }
    ...
    if (typeSym != nullptr)
    {
        std::string name = typeSym->GetName();
        if (name == "char")   { node->SetSize(1); return; }
        if (name == "int")    { node->SetSize(4); return; }
        if (name == "float")  { node->SetSize(4); return; }
        if (name == "long")   { node->SetSize(8); return; }
        if (name == "double") { node->SetSize(8); return; }
    }
...
void cSizeOffset::Visit(cArrayDeclNode *node)
{
    ...
    if (baseType != nullptr)
    {
        elemSize = baseType->GetSize();
    }
    else if (type != nullptr)
    {
        elemSize = type->GetSize();
    }
    else
    {
        cSymbol *typeSym = node->GetTypeSymbol();
        ...
        {
            std::string name = typeSym->GetName();
            if (name == "char")   elemSize = 1;
            if (name == "int")    elemSize = 4;
            if (name == "float")  elemSize = 4;
            if (name == "long")   elemSize = 8;
            if (name == "double") elemSize = 8;
        }
    }
